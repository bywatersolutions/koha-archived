...
# Weekly events
ModRepeatingEvent( 'OAK', 1, undef, undef, $new_holiday );

my $weekly_events = GetWeeklyEvents( 'OAK' );
...
ModRepeatingEvent( 'OAK', 1, undef, undef, $new_holiday );
$weekly_events = GetWeeklyEvents( 'OAK' );
...
ModRepeatingEvent( 'OAK', undef, 6, 26, $new_holiday );

my $yearly_events = GetYearlyEvents( 'OAK' );
...
ModRepeatingEvent( 'OAK', undef, 6, 26, $new_holiday );
$yearly_events = GetYearlyEvents( 'OAK' );
...
$new_holiday->{open_hour} = 0;
ModSingleEvent( 'OAK', '2013-03-17', $new_holiday );

my $single_events = GetSingleEvents( 'OAK' );
...
ModSingleEvent( 'OAK', '2013-03-17', $new_holiday );
$single_events = GetSingleEvents( 'OAK' );
...
DelRepeatingEvent( 'OAK', 1, undef, undef );
$weekly_events = GetWeeklyEvents( 'OAK' );
is( scalar @$weekly_events, 0, 'weekly deleted' );

DelRepeatingEvent( 'OAK', undef, 6, 26 );
$yearly_events = GetYearlyEvents( 'OAK' );
is( scalar @$yearly_events, 0, 'yearly deleted' );

DelSingleEvent( 'OAK', '2013-03-17' );
